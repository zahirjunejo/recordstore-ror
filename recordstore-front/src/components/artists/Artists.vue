<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form action="" @submit.prevent="addArtist">
      ...
      <input type="submit" value="Add Artist" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-light hover:bg-green-dark block w-full py-4 text-white items-center justify-center" />
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="artist in artists" :key="artist.id" :artist="artist">
        ...
        <div class="flex items-center justify-between flex-wrap">
          ...
          <button class="bg-tranparent text-sm hover:bg-blue-600 hover:text-white text-blue border border-blue no-underline font-bold py-2 px-4 mr-2 rounded"
          @click.prevent="editArtist(artist)">Edit</button>

          <button class="bg-transprent text-sm hover:bg-red-600 text-red hover:text-white no-underline font-bold py-2 px-4 rounded border border-red"
